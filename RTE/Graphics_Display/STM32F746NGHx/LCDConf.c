...
//
static U32 _aBuffer[XSIZE_PHYS * sizeof(U32) * 3] __attribute__((at(DMA2D_BUFFER_ADDR)));
...
static void _LCD_InitController(int LayerIndex) {
  ...
  int xSize, ySize, BitsPerPixel, i;
...
static LCD_PIXELINDEX * _LCD_GetpPalConvTable(const LCD_LOGPALETTE GUI_UNI_PTR * pLogPal, const GUI_BITMAP GUI_UNI_PTR * pBitmap, int LayerIndex) {
  ...
  //
  _InvertAlpha_SwapRB((U32 *)pLogPal->pPalEntries, _pBuffer_DMA2D, pLogPal->NumEntries);
  //
...
static void _LCD_SetLayerAlpha(int LayerIndex, int Alpha) {
  HAL_LTDC_SetAlpha(&LTDC_Handle, 255 - Alpha, LayerIndex);
}
...
void HAL_LTDC_LineEvenCallback(LTDC_HandleTypeDef *hltdc) {
  ...
  for (i = 0; i < GUI_NUM_LAYERS; i++) {
    if (_aPendingBuffer[i] >= 0) {
      ...
      // Set address
      //
      HAL_LTDC_SetAddress(hltdc, Addr, i);
      //
...
int LCD_X_DisplayDriver(unsigned LayerIndex, unsigned Cmd, void * pData) {
  ...
  case LCD_X_SETORG: {
    ...
    _LCD_SetOrg(LayerIndex, p->xPos, p->yPos);
    break;
  }
  ...
  case LCD_X_SETLUTENTRY: {
    ...
    _LCD_SetLUTEntry(LayerIndex, p->Color, p->Pos);
    break;
  }
  case LCD_X_ON: {
    //
  ...
  case LCD_X_SETVIS: {
    ...
    _LCD_SetVis(LayerIndex, p->OnOff);
    break;
  }
  case LCD_X_SETPOS: {
    //
    ...
    _LCD_SetLayerPos(LayerIndex, p->xPos, p->yPos);
    break;
  }
  case LCD_X_SETSIZE: {
    //
  ...
  case LCD_X_SETALPHA: {
    ...
    _LCD_SetLayerAlpha(LayerIndex, p->Alpha);
    break;
  }
  case LCD_X_SETCHROMAMODE: {
    ...
    _LCD_SetChromaMode(LayerIndex, p->ChromaMode);
    break;
  }
  case LCD_X_SETCHROMA: {
    ...
    _LCD_SetChroma(LayerIndex, p->ChromaMin, p->ChromaMax);
    break;
  }
  default:
    r = -1;
